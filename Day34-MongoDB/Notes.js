/* Cursor: a pointer to the result set of a query. By default, the cursor is not
    ...
    MongoDB Cursor Methods
        a. toArray()
          ...
          Ex:let users = db.day33Task.find().toArray()
                users.map(user => user.product_name);
                users.map(user => [user.product_name,user.product_price,user.product_color,user.product_material])
